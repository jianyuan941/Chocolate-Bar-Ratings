...
des_analysis_dataframe <-function(
  ...
  #display result
  if(as.character(enquo(return)[2])=="NULL"){
    return()
  }else{
...
des_analysis_dataframev2 <-function(
    data_base = NULL,
    f = NULL,
    group = NULL,#optional
    x = NULL, #optional
    
    #====condition to x and f=====
    accept_negative_x = T, #optional
    accept_negative_f = T, #optional, only applicable if x = null
    ...
    #====display method===
    page = NULL, #this is to set nrow in result_display2
    sorting = F, #return data in not kbl format
    return = NULL,
    ...
    if(as.character(enquo(x)[2]) != "NULL"){
      ...
      df<-df %>%
        ...
        mutate(
          ...
          var = round(sum(!!f_col*(!!x_col - mean)^2)/(sum(!!f_col)-1),2),
          sd = sqrt(var),
          range = paste0(min(!!x_col*!!f_col)," : ",max(!!x_col*!!f_col)),
          'x(68%)' = sum(ifelse(!!x_col<=(mean+sd),!!f_col,0)) - sum(
            ifelse(!!x_col<(mean-sd),!!f_col,0)),
          'x(95%)' = sum(ifelse(!!x_col<=(mean+(2*sd)),!!f_col,0))-
            sum(ifelse(!!x_col<(mean-(2*sd)),!!f_col,0)),
      ...
      df<-df %>%
        ...
        mutate(
          ...
          var = sum((!!f_col - (sum(!!f_col)/n()))^2)/(n()-1),
          sd = sqrt(var),
          range = paste0(min(!!f_col)," : ",max(!!f_col)),
          'x(68%)' = sum(!!f_col<=(mean+sd))-sum(!!f_col<(mean-sd)),
          'x(95%)' = sum(!!f_col<=(mean+(2*sd)))-sum(!!f_col<(mean-(2*sd))),
          'x(99%)' = sum(!!f_col<=(mean+(3*sd)))-sum(!!f_col<(mean-(3*sd))),
          '99%+' = sum(!!f_col>(mean+(3*sd)))+sum(!!f_col<(mean-(3*sd)))
...
des_analysis_dataframev2_reading <- function(
    database = NULL,
    assign_name = NULL){
  
  df<- distinct(database) 
  number_of_row <- nrow(df)
  
  number_of_row
  for(i in 1:number_of_row){
    
    #Basic Definations
    category <- df[i,1] #category
    total_n <- df[i,'∑n'] #sum of n
    mean_value <- df[i, "mean"] #mean value
    range_value <- df[i,"range"] #range value
    sd_value <- df[i,"sd"] #sd value
    
    #Basic Calculations
    total_sample_within_one_sd <- df[i,'x(68%)']
    total_sample_within_one_and_two_sd <- df[i,'x(95%)'] - total_sample_within_one_sd
    total_sample_beyond_two_sd <- total_n - df[i,'x(95%)']
    
    sentence_one <- paste0(assign_name,"_", category, " is ranging from ",range_value, " (mean: ", round(mean_value,4), ", sd: ", round(sd_value,4),".")
    sentence_two <- paste0(round(total_sample_within_one_sd,4), " samples (", round((total_sample_within_one_sd/total_n)*100,2),"%) fall within one standard deviation.")
    sentence_three <- paste0(round(total_sample_within_one_and_two_sd,4), " samples (", round((total_sample_within_one_and_two_sd/total_n)*100,2), "%) fall within one and two standard deviations.")
    sentence_four <- paste0(round(total_sample_beyond_two_sd,4), " samples (", round((total_sample_beyond_two_sd/total_n)*100,2), "%) beyond two standard deviations.")
    
    result <- paste0(sentence_one, " ",
                     sentence_two, " ",
                     sentence_three, " ",
                     sentence_four)
    
    assign(paste0(assign_name, "_reading_", category), result, envir = .GlobalEnv)
  }
