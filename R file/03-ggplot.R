...
map_function <- function(
    #=== 1st part to match location column with map and defined unmatch row ===
  database = NULL,
  assign_name = NULL, # input in "" format
  #=== 2nd path for plotting ===
  joinby = NULL,
  fill = NULL #data in label for map plot
  
  
  #after run 1st part
  # can use name_no_match to find all unmatch dataset
  # can use contry_list to find all country list
  # can use name_match to get matched dataset
  
  # after run 2nd part 
  # generate ggplot
){
  #defined
  name <- if(is.null(quo_get_expr(enquo(assign_name)))){as_name(enquo(database))}else{as_name(enquo(assign_name))}
  
  joinby <- enquo(joinby)
  fill <- enquo(fill)
  
  country_list  <- ne_countries(scale = "medium", returnclass = "sf")
  country_list_frame <<- as.data.frame(country_list) 
  
  #find not match data
  no_match <- database %>% 
    filter(!(!!joinby %in% country_list$name))
  
  assign(paste0(name, "_no_match"), no_match, envir = .GlobalEnv)
  
  #matching data
  matched <- left_join(country_list, database, by = c("name" = as_name(joinby)))
  assign(paste0(name, "_match"), matched, envir = .GlobalEnv)
  ...
  return_message <- 
    if(nrow(no_match) == 0) {
      ...
      return(
        paste0(
          "Not matching samples = ", nrow(no_match)
        )
      )
    }
  
  if(is.null(quo_get_expr(fill))){
    ...
    assign(paste0(name,"_plot"), fig, envir = .GlobalEnv)
  }
