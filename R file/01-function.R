...
#install.packages("writexl")
  ...
  install.packages("lubridate", repos = "https://cloud.r-project.org") #for date 
  install.packages("hrbrthemes", repos = "https://cloud.r-project.org") #ggplot theme
  ...
  source("R file\\03-ggplot.R")
  source("R file\\04-plotly.R")
  source("R file\\custom calculation.R")
  source("R file\\05-PCA-analysis.R")
  source("R file\\06-T-SNE-analysis.R")
  source("R file\\AnovaTest.R")
...
read_file <- function(
  ...
  if(str_detect(data_base, regex("\\.rds",ignore_case = T)) == T){
  rds_df <- readRDS(file.path("rds file", data_base))
  
    trim <- str_replace_all(data_base,
...
save_function<-function(
    ...
    name = NULL, #only name without file format| ie. breaast_dataset
    type = "rds", #can change to "CSV"|"rds"|"NULL"
    return = F,  #whether want to continue write after save
    #======================================================
    #optional: if want to use can refer to Breast cancer database
    assign_name = NULL, #optional
    get_mean_sd = F #optional
){
  
  quo_name <- quo_get_expr(enquo(name))
  if(!is.null(quo_name)){
    
        if(type == "rds"){
          
          if(dir.exists("rds file")){
            file_name <- paste0(as_name(quo_name),".rds")
          ...
          if(dir.exists("csv file")){
              file_name <- paste0(as_name(quo_name),".csv")
            ...
            assign(as_name(quo_name), data_base, envir = .GlobalEnv)
        }
    ...
    if(type == "rds"){
      ...
      if(dir.exists("rds file")){
        file_name <- paste0(as_name(enquo(data_base)),".rds")
        saveRDS(data_base, file = file.path("rds file",file_name))
      }else{
        dir.create("rds file")
        file_name <- paste0(as_name(enquo(data_base)),".rds")
      ...
      if(dir.exists("csv file")){
        file_name <- paste0(as_name(enquo(data_base)),".csv")
        write.csv(data_base, file = file.path("csv file",file_name), row.names = FALSE)
      }else{
        dir.create("csv file")
        file_name <- paste0(as_name(enquo(data_base)),".csv")
  ...
  if(get_mean_sd == T){
    get_mean_sd_function (database = data_base, assign_name = assign_name)
    des_analysis_dataframev2_reading(database = data_base, assign_name = assign_name)
  }
...
try_or_else <- function(expr, alternative) {
  tryCatch(
    {
      expr   # Try the main expression
    },
    error = function(e) {
      alternative  # Run alternative if error
    }
  )
}

#Check unique value for column
row_checker <- function(
    database = NULL,
    col = NULL){
  
  sort(unique(database[,col]))
  
}


#get_mean_sd
get_mean_sd_function <- function(
    database = NULL,
    assign_name = NULL #in "" format
){
  character_name <- if(is.null(quo_get_expr(enquo(assign_name)))){""}else{assign_name}
  
  for(i in 1:nrow(database)){
    mean_value <- database[i,"mean"]
    sd_value <- database[i,"sd"]
    
    assign(paste0(assign_name,"_mean_", as.character(database[i,1])),mean_value, envir = .GlobalEnv)
    assign(paste0(assign_name,"_sd_", as.character(database[i,1])),sd_value, envir = .GlobalEnv)
...
#generate new databaase base on category
filter_to_generate_new_db<- function(
    database = NULL,
    filter_col = NULL){
  
  name_database <- as_name(enquo(database))
  
  name_filter <- as_name(enquo(filter_col))
  filter_category <- unique(database[[name_filter]])
  
  database %>% filter(database[[name_filter]] == as_name(filter_category[2]))
  for(i in 1:length(filter_category)){
    
    filter_set <- database %>% filter(database[[name_filter]] == as_name(filter_category[i]))
    assign(paste0(name_database,"_categoried_by_",name_filter,"_",as_name(filter_category[i])), as.data.frame(filter_set), envir = .GlobalEnv)
  }
  
  
  return(database)
}
#TO GENERATE RANKING FOR X-AXIS
break_generator <- function(
    breaks_name = NULL, #need to in character format ""
    labels_name = NULL, #need to in character format ""
    start = NULL, #start value of x-axis 
    end = NULL, #largest value of x-axis
    gap = NULL
){
  if(!is.null(breaks_name)){
    #breaks generator
    df <- rep(seq(start,end,by =gap))
    assign(breaks_name, df, envir =.GlobalEnv)
  }
  ...
  if(!is.null(labels_name)){
    ...
    upper <- 0
    label <- c()
    for(i in start:length(df)){
      if(upper+gap <= end){
        
        lower <- if(upper == 0){start}else{upper}
        upper <- lower + gap
        result <- paste0(lower,"-", upper)
        label <- c(label, result)
      }    
    }
    assign(labels_name, label, envir =.GlobalEnv)
  }
}
